...
* **Standardization & Knowledge (KOIOS):** Enforcing standards for code, documentation, logging, and knowledge management, **crucial for system trust and verifiable operations.** This includes a robust Script Health & Management system (see details below).
...
- **Cross-Reference Visualization System:** Accessible at `/system-explorer/visualization`, this interactive network visualization shows the relationships between files in the EGOS ecosystem, with filtering capabilities by file type, subsystem, and connection count. The dashboard also includes a Cross-Reference Validator component that displays validation results and helps identify broken references.
...
## 🩺 Script Health & Management System

To ensure the reliability, maintainability, and adherence to standards across the growing EGOS script ecosystem, a dedicated Script Health & Management system has been implemented. This system focuses on:

1.  **Automated Reference Management:**
    *   The `update_script_references.py` tool automatically scans and updates Python scripts to ensure they correctly reference the central `script_management_best_practices.md` document. This has led to **>99% reference coverage**.
    *   Detailed reports (`script_reference_update_report.md` and `.json`) are generated, providing insights into coverage and any errors.

2.  **Standardized Visual Outputs:**
    *   A common HTML template (`egos_html_template.html`) ensures all generated reports and visualizations share a consistent, professional EGOS branding.
    *   The `update_visualization_templates.py` script helps in migrating existing or new HTML files to this standard template.

3.  **Continuous Integration & Validation (CI/CD):**
    *   A GitHub Actions workflow (`.github/workflows/script_validation.yml`) is in place to automatically validate scripts on every push or pull request to `main` and `develop` branches.
    *   This workflow performs checks such as:
        *   Running the (upcoming) `script_validator.py` for code quality and standards adherence.
        *   Generating script ecosystem visualizations.
        *   Verifying script reference coverage.
    *   Validation reports and visualizations are uploaded as build artifacts, providing immediate feedback on script health.

4.  **Script Status Dashboard (In Progress):**
    *   An HTML dashboard (`docs_egos/visualizations/script_status_dashboard.html`) is under development to provide a centralized view of the status of all key scripts, including validation results, reference coverage, and links to documentation and strategic analysis (inspired by user-provided insights on productizing individual scripts).

This system is crucial for upholding the **KOIOS** principle of Standardization & Knowledge and supports the overall goal of creating a resilient, transparent, and high-quality software ecosystem.
