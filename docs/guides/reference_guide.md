...
```bash
python scripts/sync_docs.py --check-only docs_egos/
```
...
python scripts/sync_docs.py --check-only docs_egos/koios/
```

- **Configuration:** Relies on schemas defined in `docs_egos/schemas/`.
...
python scripts/fix_documentation_linting.py --check-only docs_egos/**/*.md
...
python scripts/fix_documentation_linting.py --apply docs_egos/koios/standards.md
```
...
python scripts/migrate_legacy_docs.py --analyze archive/old_system_docs_egos/

# (Hypothetical) Apply suggested frontmatter to the analyzed documents
# python scripts/migrate_legacy_docs.py --apply-frontmatter archive/old_system_docs_egos/
```
...
python scripts/documentation_metrics.py --generate docs_egos/

# (Hypothetical) Generate metrics and output to a JSON file
# python scripts/documentation_metrics.py --generate docs_egos/ --output metrics_report.json
```
...
- **Frontmatter & Schemas:** All standard EGOS documents require YAML frontmatter validated against schemas (`docs_egos/schemas/`). This metadata drives automation.
...
- **Cross-Referencing (`<!-- TO_BE_REPLACED -->`)**: Use standard wiki-style links for internal references. Validation is performed by `sync_docs.py`.
...
2. Create a new schema file in `docs_egos/schemas/` (use existing schemas as templates)
...
Example new schema file (`docs_egos/schemas/tutorial_schema.yaml`):
...
1. Create the template in `docs_egos/templates/` with appropriate frontmatter
...
1. **Missing Required Fields:**
   ...
   - Reference the appropriate schema in `docs_egos/schemas/` to see all required fields
...
1. **Automated Fixes:**
   ...
   ```bash
   # For Markdown files
   python scripts/fix_documentation_linting.py --apply docs_egos/path/to/file.md
