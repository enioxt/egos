 ...
 * 
 * Finds dead code across any TypeScript codebase:
 ...
 * 
 * Migrated from egos-lab (zero external deps, Node stdlib only).
 */
...
async function deadCodeDetect(ctx: RunContext): Promise<Finding[]> {
  ...
  for (const exp of allExports) {
    ...
    if (exp.name.startsWith('use') && exp.kind === 'function') continue;
    if (exp.name === 'metadata' || exp.name === 'config') continue;
  ...
  if (ctx.mode === 'execute' && findings.length > 0) {
    ...
    const reportPath = join(reportDir, 'dead-code.md');
    const lines = [
      `# Dead Code Report`,
      ``,
    ...
    log(ctx, 'info', `Report written to docs/reports/dead-code.md`);
  }
