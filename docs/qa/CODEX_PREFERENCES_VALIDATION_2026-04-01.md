...
- Estado real do repositório: **não existe script `ssot:check`** em `package.json`.
- Comandos existentes e corretos para o mesmo objetivo:
...
- Ajustar dois pontos operacionais no seu template pessoal:
  1. Trocar `npm run ssot:check` por `bun run governance:check`.
  2. Tratar `CLAUDE.md` como "se existir no repo".
