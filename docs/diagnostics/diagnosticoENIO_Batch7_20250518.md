...
@references:
  - "<!-- TO_BE_REPLACED -->"
  - "<!-- TO_BE_REPLACED -->" # Note: HARMONY subsystem dir was found missing.
  - "<!-- TO_BE_REPLACED -->"
---
...
2.  **`CRN_roadmap.md` & `EGO_CRN_ROADMAP.md`:**
    ...
    *   **Issues:** Both contain multiple redundant YAML frontmatter blocks. `@references` and inline links are mostly outdated or incorrect, pointing to global files or wrong relative paths (e.g., `<!-- TO_BE_REPLACED -->` instead of a local `CRN_description.md`).
...
3.  **`HRM_cross_platform_compatibility.md`:**
    *   **Content:** Discusses strategies for handling differences in file paths, line endings, environment variables, and managing dependencies across Windows, Linux, macOS. References `docs_egos/process/dynamic_import_resilience.md`.
...
*   **Standard Cleanup:**
    ...
    *   Investigate the status of `docs_egos/process/dynamic_import_resilience.md` referenced in `HRM_cross_platform_compatibility.md`.
