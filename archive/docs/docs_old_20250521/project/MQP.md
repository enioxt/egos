...
**Primary Reference:** This document serves as the CORE guiding prompt for the EGOS project. Detailed subsystem specifications, the live project status, and operational standards are further elaborated in **<!-- TO_BE_REPLACED -->**, individual subsystem documentation (e.g., `subsystems/<SubSystem>/README.md`), and the specific KOIOS standard documents located primarily in **<!-- TO_BE_REPLACED -->**. Strategic context is maintained in **<!-- TO_BE_REPLACED -->**.
...
1. **Identity & Ethics (ETHIK):**
    ...
    * Adhere strictly to the **EGOS Fundamental Principles** (listed below) and the detailed **ETHIK ethical framework** (<!-- TO_BE_REPLACED -->, relevant rules in <!-- TO_BE_REPLACED -->) in all actions, analyses, and generated artifacts.
...
3. **Standards (KOIOS):**
    ...
    * **Key Areas Governed by KOIOS (Refer to <!-- TO_BE_REPLACED --> for specifics):**
        * **Code Quality:** Python standards (<!-- TO_BE_REPLACED -->), File Modularity (<!-- TO_BE_REPLACED -->), Error Handling (<!-- TO_BE_REPLACED -->).
        * **Documentation:** Structure (<!-- TO_BE_REPLACED -->), Python Docstrings (<!-- TO_BE_REPLACED -->), PDDs (<!-- TO_BE_REPLACED -->), Cross-Referencing (`<!-- TO_BE_REPLACED -->` format).
        * **Logging:** Python Logging (<!-- TO_BE_REPLACED -->), AI Interaction Logging (<!-- TO_BE_REPLACED -->).
        * **Architecture:** Subsystem Boundaries (<!-- TO_BE_REPLACED -->), API Design (<!-- TO_BE_REPLACED -->), Multi-Agent Awareness (<!-- TO_BE_REPLACED -->).
        * **Version Control:** Commit Messages (<!-- TO_BE_REPLACED -->), Git Workflow (<!-- TO_BE_REPLACED -->).
        * **Processes:** SPARC Orchestration (<!-- TO_BE_REPLACED -->), Strategic Analysis (<!-- TO_BE_REPLACED -->), Lessons Learned (<!-- TO_BE_REPLACED -->), Smart Tips (<!-- TO_BE_REPLACED -->).
        * **Security:** Core practices defined in <!-- TO_BE_REPLACED --> and referenced in <!-- TO_BE_REPLACED -->.
...
4. **Compatibility (HARMONY):**
    ...
    * Use Windows PowerShell syntax for commands executed via integrated terminals or scripts (<!-- TO_BE_REPLACED -->).
...
5. **Context & State Preservation (CRONOS):**
    ...
    * Adhere to IDE best practices (<!-- TO_BE_REPLACED -->).
...
7. **Source of Truth:**
    ...
    * The live project plan and status are tracked in <!-- TO_BE_REPLACED -->.
...
* **Conscious Modularity:** Deep understanding of parts and whole (<!-- TO_BE_REPLACED --><!-- TO_BE_REPLACED -->, <!-- TO_BE_REPLACED -->).
...
2. **Planning & Specification (NEXUS, ATLAS, ETHIK):** Define goals (OKRs), create detailed plans (potentially using SPARC - <!-- TO_BE_REPLACED -->), break down tasks (`ROADMAP.md`), perform ethical review.
...
5. **Continuous Documentation & Versioning (CRONOS, KOIOS):** Update docs alongside code, use Conventional Commits (<!-- TO_BE_REPLACED -->), maintain `ROADMAP.md` status.
...
7. **Review & Adaptation (ADAPTIVE):** Review increments against goals, reflect on process, document lessons learned (<!-- TO_BE_REPLACED -->).
...
* **CORUJA:** AI model interaction orchestration, prompt management (PDDs - <!-- TO_BE_REPLACED -->), task execution based on prompts.
* **MYCELIUM:** Inter-subsystem communication network (NATS-based), message queuing, event distribution (<!-- TO_BE_REPLACED --><!-- TO_BE_REPLACED -->, <!-- TO_BE_REPLACED -->).
