...
As of this date, the EGOS project comprises several key subsystems, each with specific roles. The documentation for these subsystems is primarily located in `docs_egos/subsystems/`. Key subsystems include:
...
*   **Root Directory (`C:\EGOS\`):** Contains main project files (`README.md`, `ROADMAP.md`, `LICENSE`), core scripts (`scripts/`), application code (`apps/`), and the main documentation directory (`docs_egos/`).
*   **`docs_egos/` Directory:** Intended as the central hub for all project documentation. Current reorganization efforts are underway to streamline this directory (<!-- TO_BE_REPLACED -->`diagnosticoENIO.md`, `DOCS_DIRECTORY_DIAGNOSTIC_*.md`).
...
Several standards and processes govern development within EGOS. These are typically documented in `docs_egos/standards/` and `docs_egos/process/` (or `docs_egos/governance/` post-reorganization).
...
*   **Core Principles:** [MQP.md](mdc:docs_egos/core_materials/MQP.md)
*   **Project Planning:** [ROADMAP.md](mdc:ROADMAP.md)
*   **Setup:** [Complete Setup Guide](mdc:docs_egos/setup/COMPLETE_SETUP.md), [Developer Environment Guide](mdc:docs_egos/setup/DEVELOPER_ENVIRONMENT.md)
*   **Contributing:** [CONTRIBUTING.md](mdc:CONTRIBUTING.md), [CODE_OF_CONDUCT.md](mdc:CODE_OF_CONDUCT.md)
*   **Standards:** [docs_egos/standards/](mdc:docs_egos/standards/) (contains SDRE, SEGURO, GTMI, etc.)
*   **Processes:** [docs_egos/process/](mdc:docs_egos/process/) (contains MWCP, cross-reference management, etc.)
*   **Cross-Reference Guide:** [cross_reference_guidelines.md](mdc:docs_egos/core/principles/cross_reference_guidelines.md)
...
*This document was automatically generated based on project READMEs and ROADMAP as of 2025-05-12. Further detail requires exploring individual subsystem documentation located in `docs_egos/subsystems/`.*