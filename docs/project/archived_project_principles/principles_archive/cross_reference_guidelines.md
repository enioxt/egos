...
*   From a file in `docs_egos/subsystems/CORUJA/` linking to a file in `docs_egos/core/principles/`:
    `../../core/principles/cross_reference_guidelines.md`
*   From a file in `docs_egos/processes/` linking to a file in `scripts/tools/`:
    `../../scripts/tools/data_importer.py`
*   From a file in `docs_egos/subsystems/MYCELIUM/` linking to a file in `apps/dashboard/`:
...
*   `generate_global_index.py`: Creates/updates the `docs_egos/INDEX.md` file.

Further details on these tools will be available in their respective documentation within `docs_egos/06_TOOLS_REFERENCE/`.
