...
This document outlines the conventions used for internationalizing the EGOS project website (`docs_egos/index.html`).

## 1. Locale Files

- Translation strings are stored in JSON files within the `docs_egos/locales/` directory.
...
- The JavaScript logic (`applyTranslations` function in `docs_egos/js/script.js`) specifically looks for these `data-i18n-*` attributes and updates the corresponding target attribute on the element.
...
- The core logic resides in `docs_egos/js/script.js` within the `DOMContentLoaded` event listener.
- The `fetchTranslations(lang)` function asynchronously loads the `/docs_egos/locales/${lang}.json` file.
