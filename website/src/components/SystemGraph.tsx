...
import { EnrichedGraphData, GraphNode, GraphEdge, applyFiltersToData, getEnrichedGraphData } from '@/utils/graphDataUtils';
...
const GraphEvents: FC<GraphEventsProps> = ({ data, setHoveredNode, filters }) => {
...
const SystemGraph: FC<SystemGraphProps> = ({ filters, enrichedGraphData: propGraphData, isLoading: propLoading, error: propError }) => {
    // Use data from props or fetch it directly
    const [localGraphData, setLocalGraphData] = useState<EnrichedGraphData | null>(propGraphData || null);
    const [isLoading, setIsLoading] = useState<boolean>(propLoading || false);
    const [error, setError] = useState<string | null>(propError || null);
    
    // If no graph data is provided via props, fetch it
    useEffect(() => {
        if (!propGraphData && !localGraphData) {
            try {
                // Import and use the enriched graph data
                const data = getEnrichedGraphData();
    ...
    }, [propGraphData, localGraphData]);
    
    // Apply filters to the graph data when filters change
    const filteredData = useMemo(() => {
        if (!localGraphData) return null;
        if (!filters) return localGraphData;
        
        return applyFiltersToData(localGraphData, filters);
    }, [localGraphData, filters]);
    
    // Use filtered data or fall back to the original data
    const displayData = filteredData || localGraphData;
    ...
    return (
        <div className="relative w-full h-full">
          ...
          >
              <GraphEvents data={displayData} setHoveredNode={setHoveredNode} filters={filters} />
