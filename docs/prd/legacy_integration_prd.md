...
*   Strategy for integrating processed artifacts into the live EGOS structure (e.g., `/docs_egos/legacy`).
...
*   **FR2: Legacy Inventory Management:** A central inventory (`docs_egos/legacy/legacy_inventory.md`) must be created and maintained, tracking artifact path, type, estimated date, language, status (raw, processed, translated, integrated), priority, and relevant tags.
...
*   **FR5: Artifact Integration:** A strategy and process must exist for placing processed artifacts into the live EGOS structure (e.g., `/docs_egos/legacy/`, `/docs_egos/archive/`, potentially linked from relevant current docs). Integration must consider discoverability.
*   **FR6: Evolution Narrative:** A dedicated document (`docs_egos/process/legacy_integration_strategy.md` or similar) must be created summarizing the project's history, the legacy integration process itself, and linking to key findings or integrated artifacts.
...
*   **Task ID: LEGACY-SCAN-01: Define Scope & Inventory Legacy Artifacts**
    ...
    *   **Key Output:** `docs_egos/legacy/legacy_inventory.md`
