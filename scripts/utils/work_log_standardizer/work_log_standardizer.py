...
log_dir = Path("C:/EGOS/logs")
log_dir.mkdir(parents=True, exist_ok=True)
log_file = log_dir / f"work_log_standardizer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
# Script version for reporting
SCRIPT_VERSION = "1.0.0"

# Constants
# =========================================================================
# Integration constants
INTEGRATION_ENABLED = True  # Enable integration with other EGOS systems
...
TOOL_REGISTRY_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
...
class WorkLogStandardizer:
    ...
    def __init__(self, active_dir: Path = DEFAULT_ACTIVE_DIR, archive_dir: Path = DEFAULT_ARCHIVE_DIR, dry_run: bool = False):
        ...
        self.archived_files_details = []  # Track details of archived files
    ...
    def run_standardization(self) -> Dict[str, int]:
        ...
        report_path = self._generate_standardization_report()
        ...
        return {  # Return summary statistics for programmatic use
            ...
            'report_path': report_path  # Include path to the generated report
        }
        
    def _generate_standardization_report(self) -> Optional[Path]:
        ...
        Returns:
            Optional[Path]: Path to the generated report file, or None if generation failed.
        """
        ...
        report_content.append(f"**Script Version:** {SCRIPT_VERSION}")
        ...
        # Add reformatting failures section if any occurred
        if self.reformatting_failures:
            report_content.append("## Reformatting Failures")
            report_content.append("The following files could not be reformatted:")
            report_content.append("")
            report_content.append("| File | Reason |")
            report_content.append("| ---- | ------ |")
            
            for failure in self.reformatting_failures:
                file_path = failure.get('file', 'Unknown')
                reason = failure.get('reason', 'Unknown reason')
                report_content.append(f"| `{file_path}` | {reason} |")
        ...
        # Add archived files section if any were archived
        if self.archived_files_details:
            report_content.append("## Files Archived")
            report_content.append("The following files were archived:")
            report_content.append("")
            report_content.append("| File | Destination | Date |")
            report_content.append("| ---- | ----------- | ---- |")
            
            for detail in self.archived_files_details:
                file_name = Path(detail.get('file', 'Unknown')).name
                destination = detail.get('destination', 'Unknown')
                archive_date = detail.get('date', 'Unknown')
                report_content.append(f"| `{file_name}` | `{destination}` | {archive_date} |")
        ...
        if not self.dry_run:
            try:
                ...
                logger.debug("Report content preview:\n" + full_report[:500] + "...")
                return None
        else:
    ...
    def _archive_work_log(self, file_path: Path) -> None:
        ...
        # Track archive operation for reporting
        self.archived_files_details.append({
            'file': str(file_path),
...
def main():
    ...
    parser.add_argument(
        "--no-integration",
        action="store_true",
        help="Disable integration with other EGOS systems"
    )
    parser.add_argument(
        "--register-only",
        action="store_true",
        help="Only register with tool registry and exit"
    )
    parser.add_argument(
        "--file",
        type=str,
        help="Process a single work log file instead of a directory"
    )
    parser.add_argument(
        "--ecosystem-analysis",
        action="store_true",
        help="Provide metadata for ecosystem analysis and exit"
    )
    ...
    # Register with tool registry if integration is enabled
    if INTEGRATION_ENABLED and not args.no_integration:
        register_with_tool_registry()
        
        # If register-only flag is set, exit after registration
        if args.register_only:
            logger.info("Registration complete, exiting as requested")
            return 0
    
    # If ecosystem-analysis flag is set, provide metadata and exit
    if args.ecosystem_analysis:
        metadata = provide_tool_metadata()
        # Ensure proper JSON output with clean formatting
        json_output = json.dumps(metadata, indent=2, sort_keys=False)
        # Use a clear print statement that won't be affected by logging
        sys.stdout.write(json_output + "\n")
        sys.stdout.flush()
        return 0
    
    # If file argument is provided, process a single file
    if args.file:
        ...
        result = standardize_work_log(file_path)
    ...
    for key, value in summary.items():
        if key != 'report_path':  # Skip report_path in the summary table
    ...
    # Inform user about the report location
    if summary.get('report_path'):
        if args.dry_run:
            print(f"\nA detailed report would be generated at: {summary['report_path']}")
        else:
            print(f"\nA detailed report has been generated at: {summary['report_path']}")
    else:
        print("\nNo detailed report was generated. Check logs for any errors.")
        
    # Potentially exit with a non-zero status code if issues were found
    # sys.exit(1) 

def get_integration_capabilities():
    """Return the integration capabilities of this tool.
    
    This method defines what services the Work Log Standardizer provides to other tools
    and what services it consumes from other tools. Following EGOS_PRINCIPLE:Conscious_Modularity,
    this enables the tool to be properly integrated into the EGOS ecosystem.
    
    Returns:
        dict: Dictionary of integration capabilities
    """
    return {
        "provides": [
            "work_log_validation",      # Validates work log files against standards
            "work_log_standardization", # Standardizes work log files
            "work_log_archiving"        # Archives completed work logs
        ],
        "consumes": [
            "file_system_events",       # Responds to file system changes
            "ecosystem_analysis"        # Provides data to ecosystem analyzer
        ],
        "hooks": {
...
def handle_integration_event(event_type, event_data):
    """Handle integration events from other tools.
    
    This method responds to events from other tools in the EGOS ecosystem.
    It enables the Work Log Standardizer to be triggered by file system events,
    ecosystem analysis, and other integration points.
    
    Args:
        event_type (str): Type of event (e.g., 'file_system_event', 'ecosystem_analysis')
        event_data (dict): Data associated with the event
        
    Returns:
        dict: Result of handling the event
    """
    logger.info(f"Handling integration event: {event_type}")
    
    if event_type == "file_system_event":
        if event_data.get("file_type") == "work_log":
            # Handle work log file change
            file_path = event_data.get("file_path")
            if file_path:
                logger.info(f"Standardizing work log due to file system event: {file_path}")
                return standardize_work_log(file_path)
    
    elif event_type == "ecosystem_analysis":
        # Provide metadata for ecosystem analysis
        return provide_tool_metadata()
    
    logger.warning(f"Unhandled integration event type: {event_type}")
    return {"status": "error", "message": f"Unhandled event type: {event_type}"}

def standardize_work_log(file_path):
    """Standardize a specific work log file.
    
    This method is called in response to file system events to standardize
    a specific work log file that has been created or modified.
    
    Args:
        file_path (str): Path to the work log file to standardize
        
    Returns:
        dict: Result of standardization
    """
    logger.info(f"Standardizing work log: {file_path}")
    ...
    try:
        ...
        # Create standardizer with default settings
        standardizer = WorkLogStandardizer()
        
        # Validate the work log
        validation_result = standardizer._validate_work_log(file_path)
        
        if validation_result:
            logger.info(f"Work log is valid: {file_path}")
            # Check if work log should be archived
            if standardizer._should_archive_work_log(file_path):
                standardizer._archive_work_log(file_path)
                return {"status": "success", "action": "archived", "file": str(file_path)}
            return {"status": "success", "action": "validated", "file": str(file_path)}
        else:
            logger.warning(f"Work log has issues: {file_path}")
            # Attempt to standardize the file
            if standardizer._standardize_work_log(file_path):
                return {"status": "success", "action": "standardized", "file": str(file_path)}
            else:
                return {"status": "error", "action": "failed_to_standardize", "file": str(file_path)}
    except Exception as e:
        logger.error(f"Error standardizing work log {file_path}: {e}")
        return {"status": "error", "message": str(e), "file": str(file_path)}

def provide_tool_metadata():
    """Provide metadata about this tool for ecosystem analysis.
    
    This method is called by the Script Ecosystem Analyzer to gather information
    about the Work Log Standardizer for system-wide analysis.
    
    Returns:
        dict: Metadata about the Work Log Standardizer
    """
    logger.info("Providing tool metadata for ecosystem analysis")
    ...
    try:
        ...
        dir_structure = {
            ...
            "files": [f.name for f in script_dir.glob("*") if f.is_file()],
            "directories": [d.name for d in script_dir.glob("*") if d.is_dir()]
        }
    except Exception as e:
    ...
    metadata = {
        ...
        "stats": {
            "files_processed": 0,  # These would be populated with actual stats in a real implementation
        ...
        "statistics": {
            ...
            "method_count": 15  # Approximate count of methods in the script
        },
        "directory_structure": dir_structure,
        "dependencies": [
            "os", "sys", "re", "json", "yaml", "logging", "argparse", 
...
def register_with_tool_registry():
    """Register the Work Log Standardizer with the EGOS tool registry.
    
    This ensures the tool is properly integrated with the EGOS ecosystem and can be
    discovered by other tools and systems. Following EGOS_PRINCIPLE:Systemic_Cartography,
    this makes the tool visible and accessible throughout the system.
    
    Returns:
        bool: True if registration was successful, False otherwise
    """
    logger.info("Registering Work Log Standardizer with tool registry")
    
    if not os.path.exists(TOOL_REGISTRY_PATH):
        logger.warning(f"Tool registry not found at {TOOL_REGISTRY_PATH}")
        return False
    ...
    try:
        ...
        tool_id = "work_log_standardizer"
        
        for tool in registry.get("tools", []):
            if tool.get("id") == tool_id:
                logger.info("Work Log Standardizer already registered in tool registry")
                return True
        ...
        tool_entry = {
            "id": tool_id,
            "name": "Work Log Standardizer",
            "description": "Standardizes work log files in the EGOS system",
            "path": script_path,
            "category": "documentation",
            "tags": ["work_logs", "standardization", "documentation"],
            "version": SCRIPT_VERSION,
            "last_updated": datetime.now().strftime("%Y-%m-%d"),
            "dependencies": ["yaml"],
            "integration": capabilities,
            "documentation": "C:\\EGOS\\docs\\tools\\work_log_standardizer.md"
        }
        ...
        logger.info("Successfully registered Work Log Standardizer with tool registry")
        return True
    except Exception as e:
        logger.error(f"Error registering with tool registry: {e}")
        return False
