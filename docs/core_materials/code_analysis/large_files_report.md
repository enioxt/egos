...
Based on our <!-- TO_BE_REPLACED -->, we analyzed our codebase to identify Python files that significantly exceed our recommended size guidelines (300-500 lines). This report documents the findings and provides insights on maintaining modularity in our own code.

## Analysis Method

We used the KOIOS file size analysis tool documented in <!-- TO_BE_REPLACED --> to identify the largest files by size. The command was:
...
Refer to the <!-- TO_BE_REPLACED --> for general techniques like Extract Class and Extract Method.
...
2. **Process**:
   ...
   - Apply modular design patterns (Extract Class, Extract Method) as described in our <!-- TO_BE_REPLACED -->.
